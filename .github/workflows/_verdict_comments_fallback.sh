...
for attempt in $(seq 1 "$MAX_TRIES"); do
  ...
  # Issue #625 fix: jq regex flags go OUTSIDE the quoted regex string,
  # not concatenated inside it. The previous inline "; i" was treated
  # as a literal character so the match failed for every bot login.
  #
  # SECURITY: anchors per alternation branch — `^(claude|anthropic)$`
  # alone would accept "iamclaudette" or "anthropic-fan". The previous
  # unanchored `claude|anthropic|...` was the #244 source of trust-bound
  # creep. Per-branch anchors defeat substring injection while keeping
  # the character-class-free form that jq accepts without jq-specific
  # bracket-escape quirks (verified by tests/test_verdict_comments_fallback.py).
