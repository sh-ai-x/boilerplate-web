...
export function GoogleSignInButton({ redirectTo, className, label = 'Continue with Google' }: { redirectTo?: string; className?: string; label?: string }) {
  ...
  return (
    <div className={className}>
      <button type="button" onClick={onClick} disabled={loading} aria-label="Sign in with Google" data-testid="google-signin-button">{loading ? 'Redirecting…' : label}</button>
      {error ? <p role="alert" data-testid="google-signin-error">{error}</p> : null}
    </div>
  );
}
